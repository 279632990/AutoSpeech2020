...
from CONSTANT import NUM_MFCC, FFT_DURATION, HOP_DURATION,LR_FFT_DURATION,LR_HOP_DURATION
...
@timeit
def extract_mfcc_parallel(data, sr=16000, n_fft=None, hop_length=None, n_mfcc=NUM_MFCC,lr=False):
    if n_fft is None:
        if lr:
            n_fft = int(sr*LR_FFT_DURATION)
        else:
            n_fft = int(sr*FFT_DURATION)
    if hop_length is None:
        if lr:
            hop_length = int(sr*LR_HOP_DURATION)
        else:
...
@timeit
def extract_melspectrogram_parallel(data, sr=16000, n_fft=None, hop_length=None, n_mels=40, use_power_db=False,lr=False):
    if n_fft is None:
        if lr:
            n_fft = int(sr*LR_FFT_DURATION)
        else:
