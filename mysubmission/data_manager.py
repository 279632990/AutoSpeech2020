...
from data_augmentation import noise, shift, stretch, pitch, dyn_change, speed_npitch,mix_up
...
class DataManager:
    ...
    def get_train_data(self, train_loop_num, model_num,
        ...
        # data augmention
#         train_test_ratio = self._metadata[TRAIN_NUM]/self._metadata[TEST_NUM]
#         if train_test_ratio<1.0:
#             cnt = len(self._pre_train_x)
#             self._pre_train_x, self._pre_train_y = self.get_augmention_data(self._pre_train_x, self._pre_train_y,
#                                                                             ratio=0,mix_ratio=min(0.25,1-train_test_ratio),
#                                                                             origin=True)
#             log(f'Data augmention, from {cnt} to {len(self._pre_train_x)} samples')
#         print('train_loop_num%s'%train_loop_num)
    ...
    @timeit
    def get_augmention_data(self, x, y, ratio=0.2, mix_ratio=1.0, origin=True):
        x_len = len(x)
        indices = range(x_len)
        ...
        if ratio>0:
            ...
            augmentions_x = np.array([noise(d) for d in np.array(x)[idxs]])
            augmention_data_x.append(augmentions_x)
            augmention_data_y.append(np.array(y)[idxs])

            # shift
            idxs = np.random.choice(indices, sample_num, replace=False)
            augmentions_x = [shift(d) for d in np.array(x)[idxs]]
            augmention_data_x.append(augmentions_x)
            augmention_data_y.append(np.array(y)[idxs])
            ...
            # dyn_change
            # idxs = np.random.choice(indices, sample_num, replace=False)
            # augmentions_x = [dyn_change(d) for d in np.array(x)[idxs]]
            # augmention_data_x.append(augmentions_x)
            # augmention_data_y.append(np.array(y)[idxs])

            # speed_npitch
            # idxs = np.random.choice(indices, sample_num, replace=False)
            # augmentions_x = [speed_npitch(d) for d in np.array(x)[idxs]]
            # augmention_data_x.append(augmentions_x)
            # augmention_data_y.append(np.array(y)[idxs])
