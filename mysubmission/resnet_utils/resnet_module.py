...
import resnet_utils.resnet_backbone as backbone
...
#     backbone_model = keras.models.Model(inputs, x)
#     backbone_model_json = backbone_model.to_json()
#     with open('./backbone.json', 'w') as file:
#         file.write(backbone_model_json)
#     t1=time.time()
#     backbone_net = model_from_json(open('./backbone.json', 'r').read())
#     inputs, x = backbone_net.input,backbone_net.output
#     t2=time.time()
#     print(t2-t1)
    ...
    x_fc = keras.layers.Conv2D(bottleneck_dim, (7, 1),
                               ...
                               name='x_fc')(x)
#     if num_class>37:
#         x_fc = keras.layers.BatchNormalization()(x_fc)
    if aggregation == 'avg':
        if mode == 'train':
