...
from resnet_utils.resnet_module import vggvox_resnet2d_icassp
...
import resnet_utils.resnet_data_generator as generator
from resnet_utils.autospeech_eda import AutoSpeechEDA
from resnet_utils.autospeech_sampler import AutoSpSamplerNew
...
def train_lr(x, y):
    ...
    X_train, X_test, y_train, y_test = train_test_split(x, y, train_size=train_size, random_state=2333,stratify=y)
...
class Model(object):
    def __init__(self, metadata, train_output_path="./", test_input_path="./"):
        ...
        self.test_num = self.metadata.get("test_num")
        ...
        if self.train_num>1000 or self.test_num>1000 or self.class_num>30:
            self.round_idx = 1
            self.test_idx = 1
        else:
            self.round_idx = 1
    ...
    def train_resnet(self, train_dataset, remaining_time_budget=None):
        ...
        if self.model_lr is None and self.round_idx==0:
            self.model_lr = train_lr(train_dataset[0].copy(),train_dataset[1])
            self.round_idx += 1
    ...
    def test_resnet(self, test_x, remaining_time_budget=None):
        """
        ...
        if self.test_idx==0:
            self.test_idx+=1
            y_pred = predict_lr(self.model_lr,test_x)
